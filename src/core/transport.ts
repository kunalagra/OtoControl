 ...
 *
 * The one hand-maintained list this fact lives in. Each driver's `services`
 * are filtered out of this array by `servicesFor` below rather than
 ...
 * from `core/driver.ts`'s `DRIVERS`) because that direction is circular:
 * `core/driver.ts` imports the two per-driver descriptor modules, those
 * import their device classes, and both device classes import this module
 * for `TransportOpener` and `openSerialTransport`.
 */
 ...
 * `transport.ts` to import `core/driver.ts`, which imports the two
 * descriptor modules, which import `device.ts` and `sony.ts`, both of which
 * import `transport.ts` for `TransportOpener` and
 ...
 *
 * It sits here, beside the table it filters, rather than in `driver.ts`,
 * because its callers are now the two per-driver descriptor modules and
 * `core/driver.ts` imports *them*. Exporting it from there instead would
 * close a runtime cycle in which a descriptor is built, at module-evaluation
 * time, from a helper that is still in its temporal dead zone.
 */
